...
void prepareDataInt( int dataOffsetEnd,int dataOffsetBeg, int bytesToRead, __int16 *hostBytesOfData, string fileDataNames,
	__int16 *deviceBytesOfData, int blockSize, int ddmQuant, int samplesOfSignal, int samplesWithOverlap, cufftComplex *deviceDataFile1
	...
	//READ DATA
	readdataInt(dataOffsetEnd-dataOffsetBeg, dataOffsetBeg, hostBytesOfData, fileDataNames);
	//readRealData(dataOffsetEnd - dataOffsetBeg, dataOffsetBeg, bytesToRead, hostBytesOfData, fileDataNames);

	CudaSafeCall(cudaMemcpy(deviceBytesOfData, hostBytesOfData, sizeof(cufftComplex)*(dataOffsetEnd-dataOffsetBeg)*2, cudaMemcpyHostToDevice));
	...
	*elapsed_read = chrono::high_resolution_clock::now() - (begin);
	...
	numBlocks = (((dataOffsetEnd-dataOffsetBeg)*2) + blockSize - 1) / blockSize;
	copyInt2Float<< <numBlocks, blockSize >> > (deviceBytesOfData, deviceDataFile1, (dataOffsetEnd-dataOffsetBeg)*2);
	CudaCheckError();
	cudaDeviceSynchronize();
...
int main(int argc, const char* argv[]) {
	...
	__int16 *hostBytesOfData, *deviceBytesOfData;
	...
	hostBytesOfData = (__int16 *)malloc(sizeof(__int16) * 2*(dataOffsetEnd-dataOffsetBeg));
	...
	CudaSafeCall(cudaMalloc(&deviceBytesOfData, sizeof(__int16)*2*(dataOffsetEnd-dataOffsetBeg)));
	...
	long long allocatedMem =  sizeof(cufftComplex)*samplesWithOverlap + nMaxBufferSize +sizeof(__int16) * 2*(dataOffsetEnd-dataOffsetBeg)+
	...
	for (i = 0; i < numofDataLines; i++) {
		...
		prepareDataInt(dataOffsetEnd[i], dataOffsetBeg[i], bytesToRead, hostBytesOfData, fileDataNames[i],
		...
		if (interferometic == true) {
			...
			prepareDataInt( dataOffsetEnd[i]+(dataOffsetBegInterferometric[i]-dataOffsetBeg[i]), dataOffsetBegInterferometric[i],
